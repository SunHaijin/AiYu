...
#define NAVBAR_HEIGHT   ([[[UIDevice currentDevice] systemVersion] floatValue]>=7?64:44)
    ...
    if (self) {
        ...
        switch (action) {
            case PasscodeActionSet:
                ...
                _enterPrompt = NSLocalizedString(@"请输入密码", nil);
            ...
            case PasscodeActionChange:
                self.title = NSLocalizedString(@"密码更改", nil);
        ...
        case PasscodeActionSet:
            if (phase == 0) {
                ...
                if ([text isEqualToString:_passcode]) {
                    ...
                    messageLabel.text = NSLocalizedString(@"密码输入错误, 请重试", nil);
                }
            }
            break;
